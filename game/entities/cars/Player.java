...
public class Player extends Cars {
    ...
    public void tick() {
        ...
        if (points < 0) {
            if (pointsEndStall > pointsEndStallMax) {
                if (game.getFreePlay() && game.lockGetter())
                    game.restartFreePlay();
                else if (game.getFreePlay() && !game.lockGetter())
                    game.restartFreePlay();
                else if (!game.getFreePlay() && game.lockGetter())
                    game.restartFreePlay();
                else if (!game.getFreePlay() && !game.lockGetter())
                    game.restartAndIterate();
            }
            pointsEndStall++;
        }
    ...
    public void collisionCPU(List<CPU> carArray) {
        ...
        for (int i = 0; i < carArray.size(); i++) {
            ...
            for (int j = 0; j < 4; j++) {
                if ((playerCorners[j][0] >= temp.x) && (playerCorners[j][0] <= temp.x1)
                        && (playerCorners[j][1] >= temp.y) && (playerCorners[j][1] <= temp.y1)) {
                    points += -1000;
                }
            }
